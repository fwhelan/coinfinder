...
class Coincidence
{
    ...
    private:
        static std::map<double, std::pair<std::string, std::string>> calc_phylogenetic_distances( const std::string& phylogeny);
	...
	static void _coincidence_to_p( const DataSet& dataset, const Alpha& alpha_yain, const Alpha& alpha_tain, double cor_sig_level, int both_of, int one_of, int any_yain, int any_tain, const std::map<double, std::pair<std::string, std::string>>& phylo_dists, const id_lookup<Edge>& edge_table, const double max_act_phylodist, const std::vector<std::string>& edges_ovlp);

...
{
    public:
        static void run(DataSet& dataset, const std::string& phylogeny, const std::string& path, const std::string& prefix);
	...
	static double  _coincidence_to_p( const DataSet& dataset, std::ofstream& analysis, const Alpha& alpha_yain, const Alpha& alpha_tain, double cor_sig_level, int both_of, int one_of, int any_yain, int any_tain, const id_lookup<Edge>& edge_table, const std::vector<std::string>& edges_ovlp, const std::vector<std::string>& edges_union, std::string path);
        static void _write_header(const DataSet& dataset, std::ofstream& analysis);
};

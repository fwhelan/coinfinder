...
class Coincidence
{
	...
	static void _coincidence_to_p( const DataSet& dataset, const Alpha& alpha_yain, const Alpha& alpha_tain, double cor_sig_level, int both_of, int one_of, int any_yain, int any_tain, const std::map<std::pair<std::string, std::string>, double>& phylo_dists, const std::map<const Beta*, int>& edges_yain, const std::map<const Beta*, int>& edges_tain, const id_lookup<Edge>& edge_table, double max_act_phylodist);
